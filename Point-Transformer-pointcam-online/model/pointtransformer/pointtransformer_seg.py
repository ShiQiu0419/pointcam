...
#################################################################################
#################################################################################
class PointTransformerSeg_pointcam(nn.Module):
    def __init__(self, block, blocks, c=6, k=13):
        super().__init__()
        self.c = c
        self.in_planes, planes = c, [32, 64, 128, 256, 512]
        ...
        self.attentfeat1 = nn.Conv1d(35, 1, 1)
        self.attentfeat2 = nn.Conv1d(67, 1, 1)
        self.attentfeat3 = nn.Conv1d(131, 1, 1)
    ...
    def forward(self, pxo):
        p0, x0, o0 = pxo  # (n, 3), (n, c), (b)
        self.upsample = nn.Upsample(size=p0.shape[0], mode='nearest')

        ##################################################
        x0 = p0 if self.c == 3 else torch.cat((p0, x0), 1)

        attentpoint = torch.cat((p0, x0), -1)
        attentpoint = attentpoint.transpose(1, 0)

        W = self.attentpoint(attentpoint.unsqueeze(0))
        W = F.softmax(W, dim=1)
        ...
        p1, x1, o1 = self.enc1([p0, x0, o0])
        attention1 = torch.cat((p1, x1), -1)
        attention1 = attention1.transpose(1, 0)
        attention1 = self.attentfeat1(attention1.unsqueeze(0))
        attention1 = self.upsample(attention1) 
        attention1 = self.sigmoid(attention1)
        attention1 = attention1.squeeze(0).squeeze(0)

        p2, x2, o2 = self.enc2([p1, x1, o1])
        attention2 = torch.cat((p2, x2), -1)
        attention2 = attention2.transpose(1, 0)
        attention2 = self.attentfeat2(attention2.unsqueeze(0)) 
        attention2 = self.upsample(attention2)
        attention2 = self.sigmoid(attention2)
        attention2 = attention2.squeeze(0).squeeze(0)

        p3, x3, o3 = self.enc3([p2, x2, o2])
        attention3 = torch.cat((p3, x3), -1)
        attention3 = attention3.transpose(1, 0)
        attention3 = self.attentfeat3(attention3.unsqueeze(0)) 
        attention3 = self.upsample(attention3)
        attention3 = self.sigmoid(attention3)
        attention3 = attention3.squeeze(0).squeeze(0)
        #############################################
        ...
        return x, attention1, attention2, attention3, w1, w2, w3


def pointtransformer_seg_repro_pointcam(**kwargs):
    model = PointTransformerSeg_pointcam(PointTransformerBlock, [2, 3, 4, 6, 3], **kwargs)
    return model
